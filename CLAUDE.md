...
- `desktop/` 桌面端 Prisma schema（独立 `schema.desktop.prisma`）
- `docs/` 文档（API 速查、权限架构、开发计划）
- `spec.md` 技术规格说明书
...
**配置**：运行时配置（AI API 密钥、飞书凭证等）通过 `ConfigService.get(key)` 读取，存储在数据库 `Config` 表中，而非 `process.env`。
...
**前端技术栈**（`frontend/package.json`）：React + TypeScript + Vite，不使用 Ant Design Pro；使用 Framer Motion 做动画、Lucide React 做图标。
...
**桌面端 Schema**：`backend/prisma/schema.desktop.prisma` 为独立 schema，对应生成命令 `npm run -w backend prisma:generate:desktop`。
